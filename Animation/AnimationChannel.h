...
struct KeyFrameInterpolation {
    static T Interpolate(T start, T, float) {
        return start;
    }
};
...
struct KeyFrameInterpolation<glm::vec3> {
    static  glm::vec3 Interpolate(glm::vec3 start, glm::vec3 end, float t) {
...
struct KeyFrameInterpolation<glm::quat> {
    static glm::quat Interpolate(glm::quat start, glm::quat end, float t) {
...
public:
    ...
    static std::pair<keyframe_type, keyframe_type> GetSurroundingKeyframes(float time, const std::vector<keyframe_type>& keyframes) {
        if(keyframes.empty()) {
