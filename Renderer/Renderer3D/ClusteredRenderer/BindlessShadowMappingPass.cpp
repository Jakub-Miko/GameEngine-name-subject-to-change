#include "BindlessShadowMappingPass.h"
...
struct BindlessShadowMappingPass::internal_data {
	...
	std::shared_ptr<RenderResourceStore> cascaded_shadow_map_store;
	std::shared_ptr<RenderResourceStore> cubemap_shadow_map_store;
	RenderResourceCollection<glm::mat4> shadow_cascades;
};

BindlessShadowMappingPass::BindlessShadowMappingPass(const std::string& input_shadow_casters_directional, const std::string& input_shadow_casters_point, const std::string& output_dependency_tag, const std::string& output_shadow_cascades
	, const std::string& cascaded_shadowmap_store, const std::string& cubemap_shadowmap_store)
	: input_shadow_casters_directional(input_shadow_casters_directional), input_shadow_casters_point(input_shadow_casters_point),
	output_dependency_tag(output_dependency_tag), output_shadow_cascades(output_shadow_cascades),
	cascaded_shadowmap_store(cascaded_shadowmap_store), cubemap_shadowmap_store(cubemap_shadowmap_store)
{
	data = new internal_data;
	InitShadowMappingPassData();
}

void BindlessShadowMappingPass::Setup(RenderPassResourceDefinnition& setup_builder)
{
	...
	setup_builder.AddPersistentResource<std::shared_ptr<RenderResourceStore>>(cascaded_shadowmap_store, data->cascaded_shadow_map_store);
	setup_builder.AddPersistentResource<std::shared_ptr<RenderResourceStore>>(cubemap_shadowmap_store, data->cubemap_shadow_map_store);
...
BindlessShadowMappingPass::~BindlessShadowMappingPass()
{
	if (data) {
		delete data;
	}
...
void BindlessShadowMappingPass::InitShadowComponent(Entity ent, LightType light_type, std::shared_ptr<RenderCommandList> list)
{
	...
	if (light_type == LightType::DIRECTIONAL) {
		...
		list->AttachResourceToStoreAfterSubmission(data->cascaded_shadow_map_store, depth_array);
		...
		depth_text_desc.usage = TextureUsage::DEPTH_ATTACHMENT_READABLE;
...
void BindlessShadowMappingPass::RenderDirectionalShadowCaster(Entity caster, std::shared_ptr<RenderCommandList>  list)
{
	...
	if (shadow_comp.shadow_map == nullptr) {
		InitShadowComponent(caster, LightType::DIRECTIONAL, list);
	}
...
void BindlessShadowMappingPass::RenderPointShadowCaster(Entity caster, std::shared_ptr<RenderCommandList>  list)
{
	auto& world = Application::GetWorld();
	auto& shadow_comp = world.GetComponent<ShadowCasterComponent>(caster);
	auto& caster_trans_comp = world.GetComponent<TransformComponent>(caster);
	...
	if (shadow_comp.shadow_map == nullptr) {
		InitShadowComponent(caster, LightType::POINT, list);
	}
	std::vector<Entity> entities;
...
void BindlessShadowMappingPass::InitShadowMappingPassData()
{
...
void BindlessShadowMappingPass::Render_impl(RenderPipelineResourceManager& resource_manager)
{
