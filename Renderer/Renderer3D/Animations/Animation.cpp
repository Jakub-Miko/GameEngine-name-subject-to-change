...
AnimationPlayback::AnimationPlayback(std::shared_ptr<Animation> animation, std::shared_ptr<Mesh> skeletal_mesh) : anim(animation), playback_state(), skeletal_mesh(skeletal_mesh)
{
	...
	RenderBufferDescriptor desc(num_of_bones, RenderBufferType::UPLOAD, RenderBufferUsage::CONSTANT_BUFFER);
	animation_buffer = RenderResourceManager::Get()->CreateBuffer(desc);
}

bool AnimationPlayback::UpdateAnimation(float delta_time)
{
	std::vector<glm::mat4> bone_transforms;
